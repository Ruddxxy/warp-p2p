...
vi.mock("../TransferEngine", () => ({
  TransferEngine: vi.fn().mockImplementation(function (
    _url: string,
...
describe("useTransferStore", () => {
  ...
  describe("createRoom", () => {
    ...
    it("saves session to sessionStorage", async () => {
      ...
      const stored = JSON.parse(
        sessionStorage.getItem("warp-lan-session") ?? "{}",
      );
    ...
    it("handles errors and sets error state", async () => {
      ...
      (
        TransferEngine as unknown as ReturnType<typeof vi.fn>
  ...
  describe("event callbacks", () => {
    ...
    it("onFileMetadata sets fileMetadata", async () => {
      ...
      (capturedEvents.onFileMetadata as (m: typeof metadata) => void)?.(
        metadata,
      );
