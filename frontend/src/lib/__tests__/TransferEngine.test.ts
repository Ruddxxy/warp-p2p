...
vi.mock("../SignalingClient", () => ({
  SignalingClient: vi.fn().mockImplementation(function (_config: {
    onClose?: () => void;
    onError?: () => void;
  }) {
...
class MockRTCPeerConnection {
  ...
  constructor() {
    ...
    Object.defineProperty(this, "onconnectionstatechange", {
      ...
      set: (fn: (() => void) | null) => {
        mockPcOnConnectionStateChange = fn;
      },
    });
    Object.defineProperty(this, "ondatachannel", {
      get: () => mockPcOnDataChannel,
      set: (fn: ((e: { channel: MockDataChannel }) => void) | null) => {
        mockPcOnDataChannel = fn;
      },
    });
    Object.defineProperty(this, "onicecandidate", {
      get: () => mockPcOnIceCandidate,
      set: (fn: ((e: { candidate?: unknown }) => void) | null) => {
        mockPcOnIceCandidate = fn;
      },
    });
  }
}
...
async function advanceThroughHandshake(
...
describe("TransferEngine", () => {
  ...
  describe("Fix 1.2 — WebRTC connection timeout", () => {
    ...
    it("clears the timeout on stop/cleanup", async () => {
      ...
      const hasTimeoutError = errorCalls.some((call: unknown[]) =>
        (call[0] as Error).message.includes("timed out"),
      );
  ...
  describe("Fix 4.1 — File metadata validation", () => {
    ...
    it("rejects metadata with negative size", async () => {
      const { dc } = await setupReceiverWithDataChannel();
    ...
    it("sanitizes backslash path separators in filename", async () => {
      ...
      dc.onmessage?.({
        data: JSON.stringify({
          type: "metadata",
          metadata: {
            name: "..\\..\\windows\\system32\\evil.exe",
            size: 100,
            type: "application/octet-stream",
          },
        }),
  ...
  describe("Fix 2.4 — Event-driven backpressure", () => {
    const BUFFER_THRESHOLD = 64 * 1024 * 1024; // 64MB — must match TransferEngine constant
    ...
    it("sets bufferedAmountLowThreshold and waits for onbufferedamountlow", async () => {
      ...
      // Open the channel — this triggers startBatchSend (sends batch info)
      dc.onopen?.();
      await vi.advanceTimersByTimeAsync(0);

      // Respond with batch-ack
      dc.onmessage?.({ data: JSON.stringify({ type: "batch-ack" }) });
      await vi.advanceTimersByTimeAsync(0);

      // Set bufferedAmount above threshold BEFORE file-ack triggers chunk sending
      Object.defineProperty(dc, "bufferedAmount", {
        get: () => BUFFER_THRESHOLD + 1,
        configurable: true,
      });

      // Now send file-ack — engine starts sending chunks, hits high buffer immediately
      dc.onmessage?.({
        data: JSON.stringify({ type: "file-ack", fileId: "test" }),
      });
    ...
    it("rejects if channel is closed when backpressure check runs", async () => {
      ...
      // Respond with batch-ack and file-ack
      dc.onmessage?.({ data: JSON.stringify({ type: "batch-ack" }) });
      await vi.advanceTimersByTimeAsync(0);

      // Set high buffer AND closed state before triggering file-ack
  ...
  describe("Backpressure channel close", () => {
    ...
    it("rejects immediately when channel closes during backpressure wait", async () => {
      ...
      // Respond with batch-ack and file-ack to start chunk sending
      dc.onmessage?.({ data: JSON.stringify({ type: "batch-ack" }) });
      await vi.advanceTimersByTimeAsync(0);
      ...
      // Trigger file-ack to start sending — will hit waitForBufferDrain
      dc.onmessage?.({
        data: JSON.stringify({ type: "file-ack", fileId: "test" }),
      });
      await vi.advanceTimersByTimeAsync(0);
